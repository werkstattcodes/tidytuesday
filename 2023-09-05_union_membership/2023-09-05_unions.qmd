...
Link to repo: https://github.com/rfordatascience/tidytuesday/blob/master/data/2023/2023-09-05/readme.md
...
library(camcorder)
camcorder::gg_record( 
    dir    = here::here("2023-09-05_union_membership","plots"), 
    device = "png",
    width  = 10,
    height = 10,
    units  = "in",
...
Annotations are calculated in advance to be able to change color depending on position (above bar/on the bar).
```{r}
...
pl_map <- df_states %>%
  ...
  geom_text(
    ...
    aes(
      x = year_min - 0.5,
      y = pos_max + .03,
      label = p_members_first %>% scales::percent(., accuracy = .1) 
    ),
    family="SSP",
    color=col_txt,
    hjust = 0,
  ...
  geom_text(
    ...
    aes(
      x = year_max,
      y = pos_max + .03,
      label = p_members_last %>% scales::percent(., accuracy = .1)
    ),
    family="SSP",
    color=col_txt,
    hjust = 1,
...
See https://github.com/hafen/geofacet/issues/175 
```{r}
pl_map_grob <- get_geofacet_grob(pl_map)
```

Needs to be wraped; otherwise patchwork won't work well with geofacet plot 
```{r}
...
df_demo <- demographics %>%
  ...
  mutate(facet=case_when(
    ...
    str_detect(facet, regex("finance")) ~
...
Diffent styling of facet strip for totoals; goes into facet_wrap2
```{r}
...
pl_bar_perc <- df_bar_perc %>%
  ggplot() +
  labs(
    title = "Share of union members per year and sector (1973-2022):",
    subtitle="Only in the public administration did union membership rates not decline. No data for 1982 available."
  ) +
  geom_bar(
    aes(
      x = year,
...
txt_source <- "Data: Barry T. Hirsch, David A. Macpherson, and William E. Even (www&#46;
unionstats&#46;com)"
...
pl_comb <- wrap_pl_map_grob_themed / pl_bar_perc + 
    ...
    plot_annotation(
        ...
        subtitle="Union membership rates have been declining for several decades in the US, across regions as well as <br>across sectors. Data provided on union.com details this development.",
        caption= txt_caption,
        theme=theme(
          plot.title=element_markdown(
            ...
            family="Oswald", 
            face="bold",
            margin=ggplot2::margin(2, 0, 0, .2, "cm"),
            size=20),
